...
bool CDeviceDlg::DeviceInfoCheck()
{
	...
	m_agCamera.Create(pRtcEngine);
	std::string defCameraID = getInfoManager()->getConfig()->getCameraComID(getInfoManager()->getCurSection());
	bool res = m_agCamera.SetCurDevice(s2cs(defCameraID));
	m_agCamera.Close();
...
void CDeviceDlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	int nDeviceCount = m_agCamera.GetDeviceCount();
	CString DeviceName, DeviceID;
	int nCurSel = CB_ERR;
	std::string processSection = getInfoManager()->getCurSection();
	CString defDeviceName = s2cs(getInfoManager()->getConfig()->getCameraName(processSection));
	for (int nDeviceId = 0; nDeviceCount > nDeviceId; nDeviceId++)
	{
		m_agCamera.GetDevice(nDeviceId, DeviceName, DeviceID);
		m_cbxCam.AddString(DeviceName);
		if (defDeviceName == DeviceName)
		{
			nCurSel = nDeviceId;
		}
	}
...
void CDeviceDlg::OnBnClickedBtnconfirmDevice()
{
	...
	CString curDeviceName, DeviceName, DeviceID;
	int nDeviceCount = -1;
	if (CB_ERR != nCurSel){
		m_cbxCam.GetWindowTextW(curDeviceName);
		nDeviceCount = m_agCamera.GetDeviceCount();
		for (int nDeviceId = 0; nDeviceCount > nDeviceId; nDeviceId++){

			m_agCamera.GetDevice(nDeviceId, DeviceName, DeviceID);
			if (curDeviceName == DeviceName){
				m_agCamera.SetCurDevice(DeviceID);
				break;
			}
		}
	}
