...
void CDeviceDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_RTSPCAPTURE, m_ckRtsp);
	DDX_Control(pDX, IDC_EDIT_RTSPURL, m_ctlRTSP);
}
...
void CDeviceDlg::videoExternCapture(BOOL bEnable /*= ture*/)
{
	if (!m_ckRtsp.GetCheck()){
		return;
	}

	CString rtspURL;
	m_ctlRTSP.GetWindowText(rtspURL);
	if (_T("") == rtspURL){
...
void CDeviceDlg::DrawClient(CDC *lpDC)
{
	...
	rcText.SetRect(rcClient.Width() / 2 - 190, 275, rcClient.Width() / 2 + 130, 307);
	lpDC->RoundRect(&rcText, CPoint(32, 32));
	lpDC->SetTextColor(RGB(0x00, 0x00, 0x00));
	lpString = _T("RtspURL");
	//lpDC->TextOut(200, 270, lpString, _tcslen(lpString));
