...
if [ -z "${CHANNEL_NAME}" ] || [ -z "${APP_ID}" ]; then
    ...
    echo {\"Recording_Dir\":\"`pwd`/public/${APP_ID}${CHANNEL_NAME}_jsmpeg\", \"BitrateKbps\":\"500\",\"${uid1}\":\"${push_url_1}\", \"${uid2}\":\"${push_url_2}\", \"IFrameOnly\":\"0\", \"FPS\":\"10\"} > ./public/${APP_ID}${CHANNEL_NAME}_jsmpeg/cfg.json
    ...
    if [ -z "${CHANNEL_KEY}" ]; then
        ...
        nohup ./Agora_Recording_SDK_for_Linux_FULL/samples/release/bin/recorder --appId ${APP_ID} --channel ${CHANNEL_NAME} --getVideoFrame 5 --uid ${recorder} --cfgFilePath ./public/${APP_ID}${CHANNEL_NAME}_jsmpeg/cfg.json --appliteDir `pwd`/Agora_Recording_SDK_for_Linux_FULL/bin/ > ./recorder_${CHANNEL_NAME}_${APP_ID}_jsmpeg.log 2>&1 &
    else
        ...
        nohup ./Agora_Recording_SDK_for_Linux_FULL/samples/release/bin/recorder --appId ${APP_ID} --channel ${CHANNEL_NAME} --getVideoFrame 5 --uid ${recorder} --cfgFilePath ./public/${APP_ID}${CHANNEL_NAME}_jsmpeg/cfg.json --channelKey ${CHANNEL_KEY} --appliteDir `pwd`/Agora_Recording_SDK_for_Linux_FULL/bin/ > ./recorder_${CHANNEL_NAME}_${APP_ID}_jsmpeg.log 2>&1 &
    fi
fi
